...
export default function DeleteConfirmModal({ transaction, onClose, onSuccess }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        showSuccess(t('transactions.delete.success'));
        onSuccess();
      } else {
        handleApiResponse(result, null, null, 'transaction cancellation');
      }
    } catch (err) {
      handleError(err, null, 'transaction cancellation', t('transactions.delete.unexpectedError'));
    } finally {
      setLoading(false);
    }
  };
      ...
      >
          ...
          <div className="p-6">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 mb-4 transition-colors">
              ...
              <div className="flex items-center justify-between mb-2">
                <span className="text-text-secondary text-sm">{t('transactions.common.client')}:</span>
                <span className="text-text-main font-semibold">
                  {transaction.clientCompany?.name || transaction.recipientName || '-'}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-text-secondary text-sm">{t('transaction.declarationNumber')}:</span>
                <span className="text-text-main font-semibold">
            ...
            <label className="flex flex-col w-full mb-4">
              <p className="text-text-main text-sm font-medium pb-2">{t('transactions.delete.reason')}</p>
              <textarea
                value={reason}
                ...
                placeholder={t('transactions.delete.reasonPlaceholder')}
              />
            </label>
          </div>
          ...
          <div className="flex items-center justify-end gap-4 p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 transition-colors">
            ...
            >
              ...
              {loading ? t('transactions.delete.cancelling') : t('transactions.delete.confirm')}
            </button>
          </div>
      </div>
    </div>
