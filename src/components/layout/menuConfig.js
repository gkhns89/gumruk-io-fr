...
// Bayraklar `useFeatureFlags()`'ten gelir; menüler `{ hasFeature }` geçer. Verilmezse bayraklı öğeler gizli kalır.
const NO_FLAGS = { hasFeature: () => false };
const hasShipments = (flags) => flags?.hasFeature?.(FEATURE_FLAGS.COURIER_CLIENT_STOPS) === true;
...
export const getGeneralMenuItems = (user, flags = NO_FLAGS) => [
  ...
  // Müşterinin kurye gönderileri — COURIER_CLIENT_STOPS
  ...(user?.globalRole === 'CLIENT_USER' && hasShipments(flags)
    ? [menuItem("package_2", "nav.myShipments", "/my-shipments")]
    : []),
...
export const MANAGEMENT_ITEMS = [
  ...
  menuItem("package_2", "nav.courierShipments", "/management/courier-shipments", {
    roles: ['BROKER_ADMIN', 'BROKER_USER', 'SUPER_ADMIN'],
    condition: (user, flags) => hasShipments(flags),
  }),
...
export const getVisibleManagementItems = (user, flags = NO_FLAGS) =>
  MANAGEMENT_ITEMS.filter((item) => {
    if (!item.roles.includes(user?.globalRole)) return false;
    if (item.condition && !item.condition(user, flags)) return false;
    return true;
  });
