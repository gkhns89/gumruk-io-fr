 ...
 *   createdAt, updatedAt, expiresAt, mine, target }. Zamanlar UTC (`2026-09-15T20:30:00Z`); `createdBy.fullName` kullanıcı
 *   adıdır. `payload` formun geri yüklenebilir JSON görüntüsüdür.
 *
 * İki tür taslak var. `targetId` boşsa **yeni kayıt** taslağıdır (aşama 2). Doluysa var olan bir kaydın **bekleyen
 * değişikliği**dir (aşama 3): `target` alanı hedefin şimdiki hâlini taşır —
 * `{ id, label, updatedAt, baseUpdatedAt, exists, stale }`. `baseUpdatedAt` taslak alındığı andaki kayıt sürümü,
 * `updatedAt` şimdiki sürümü; `stale` true ise kayıt taslaktan sonra başkası tarafından değiştirilmiştir ve
 * uygulamadan önce kullanıcıya gösterilir. `exists` false ise kayıt silinmiştir, taslak artık uygulanamaz.
 * Taslak kaydederken `baseUpdatedAt` modalın açıldığı andaki kayıt sürümüdür; sunucu göndermezsek kaydın o anki
 * değerini yazar.
 *
 * Hatalar: 404 `DRAFT_NOT_FOUND` (yok, silinmiş, süresi dolmuş ya da görünmüyor), 404 `DRAFT_TARGET_NOT_FOUND`
 * (taslağın kaydı silinmiş ya da başka firmanın), 400 `DRAFT_TOO_LARGE`,
...
const CODE_FALLBACK_KEYS = {
  ...
  DRAFT_TARGET_NOT_FOUND: 'api.drafts.targetNotFound',
...
// Taslağın kendisi yok ile taslağın kaydı yok ayrı şeyler: ikincisinde yeniden oluşturmak da işe yaramaz.
const TARGET_NOT_FOUND = 'DRAFT_TARGET_NOT_FOUND';

const isNotFoundError = (error) => errorCodeOf(error) !== TARGET_NOT_FOUND
  && (error?.response?.status === 404 || errorCodeOf(error) === 'DRAFT_NOT_FOUND');

/** Servis sonucu "taslak yok" mu (silinmiş, temizlenmiş, süresi dolmuş ya da görünmüyor)? */
export const isDraftNotFound = (result) => result?.code !== TARGET_NOT_FOUND
  && (result?.status === 404 || result?.code === 'DRAFT_NOT_FOUND');

/** Servis sonucu "taslağın ait olduğu kayıt yok" mu (silinmiş ya da başka firmanın)? */
export const isDraftTargetNotFound = (result) => result?.code === TARGET_NOT_FOUND;
...
export const draftService = {
  ...
  /**
   * Bir modüldeki bekleyen değişiklikler (hedefi olan taslaklar), listedeki rozetler için. Payload taşımaz.
   * Satır: { id, module, targetId, label, createdBy, updatedAt, expiresAt, mine, canApply, targetExists, targetLabel }.
   * `canApply` yalnızca kendi taslağı ve kayıt duruyorsa true; yönetici başkasınınkini görür ve siler, uygulayamaz.
   * @param {'TRANSACTION'|'WAREHOUSE'|'CARGO'} module
   */
  listPendingDrafts: async (module) => {
    try {
      const response = await axiosInstance.get('/drafts/pending', { params: { module } });
      return { success: true, data: Array.isArray(response.data) ? response.data : [] };
    } catch (error) {
      return failure('listPendingDrafts', error, 'api.drafts.listError');
    }
  },
  ...
  /**
   * Yeni taslak. `targetId` doluysa var olan bir kaydın bekleyen değişikliğidir; `baseUpdatedAt` formun açıldığı
   * andaki kayıt sürümüdür (çakışma tespiti için).
   * @param {Object} body - { module, targetId?, label?, payload, schemaVersion, baseUpdatedAt? }
   * @returns data: oluşturulan taslak
   */
  createDraft: async ({ module, targetId = null, label, payload, schemaVersion, baseUpdatedAt = null }) => {
    try {
      const response = await axiosInstance.post('/drafts',
        { module, targetId, label, payload, schemaVersion, baseUpdatedAt });
   ...
   * @param {number} id
   * @param {Object} body - { label, payload, schemaVersion, baseUpdatedAt? }
   */
  updateDraft: async (id, { label, payload, schemaVersion, baseUpdatedAt = null }) => {
    try {
      const response = await axiosInstance.put(`/drafts/${id}`,
        { label: label ?? null, payload, schemaVersion, baseUpdatedAt });
